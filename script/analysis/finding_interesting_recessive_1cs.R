...
for (  l in 1:nrow(int_df) ){
  ...
  target_gene=int_df$gene[l] #"Adipose Tissue" "Heart" "Pituitary" "Thyroid"
  ...
  temp_dir= "/project2/mstephens/wdenault/susie_mix/temp_plink/"  # avoids collisions across parallel calls
  ...
  number= table(geno[ , out[[k]]$susie_add$sets$cs$L1[[1]][1]] )
  if ( length(number)==3){

    my_main  = paste(target_gene, "Expression level in   ", tissue,
                     "  \n  n 0 variant = ",number[1], "n 1 variant = ",number[2] , "n 2 variants = " ,number[3])
  }
  if( length(number)==2){

    my_main  = paste(target_gene, "Expression level in   ", tissue,
                     "  \n  n 0 variant = ",number[1], ",n 1 variant = ",number[2] , ",n 2 variants = " ,0)
  }

  print(boxplot( pheno$y ~ SNP, main = my_main  ))


  png(file.path(plot_folder, paste0(target_gene, "_", tissue, "_boxplot.png")),
      width = 2000, height = 2000, res = 300)

  boxplot(pheno$y ~ SNP, main =my_main)
