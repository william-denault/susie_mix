# Shared additive-one-CS versus mixed-two-CS plotting.
# Source fit_mix_expression_plot_utils.R and load susieR before calling.
...
plot_four_panel_case <- function(
  ...
  susie_plot(
    fit_mix,
    y = "PIP",
    main = paste0(mixed_label, ": 2 credible sets")
  )
  ...
  mtext(paste0("ELBO + KL summary metric; positive favors ", mixed_label, "; no calibrated p-value"),
...
select_add1_mix2_cases <- function(
    res, association_threshold = 1e-8, minimum_mean_reads = 100,
    mixed_count_column = "ncs_susie_mix", overlap_column = "overlap_snp") {
  res <- as.data.table(res)
  required <- c("gene", "tissue", "min_pv", "mean_count", "ncs_susie",
                mixed_count_column, overlap_column)
  missing <- setdiff(required, names(res))
  if (length(missing)) stop("res_summary is missing: ", paste(missing, collapse = ", "), ".")
  keep <- is.finite(res$min_pv) & res$min_pv < association_threshold &
    is.finite(res$mean_count) & res$mean_count >= minimum_mean_reads &
    !is.na(res$ncs_susie) & res$ncs_susie == 1L &
    !is.na(res[[mixed_count_column]]) & res[[mixed_count_column]] == 2L &
    !is.na(res[[overlap_column]])
  columns <- unique(c(required, intersect("em_iteration", names(res))))
  cases <- unique(res[which(keep), ..columns], by = c("gene", "tissue"))
  setorder(cases, gene, tissue)
  cases
}

run_add1_mix2_plots <- function(
    cases, plot_dir,
    ...
    summary_filename = "additive_1cs_vs_mix_2cs_plot_summary.csv",
    min_maf_plink = 0.00, min_maf = 0.05, hwe_thresh = 1e-8,
    min_n_rec = 5, cis_window = 5e5, min_samples = 50,
    result_reader = NULL, mixed_fit_name = "susie_mix",
    mixed_label = "SuSiE-mix", em_iteration = NULL) {
  cases <- unique(as.data.table(cases), by = c("gene", "tissue"))
  setorder(cases, gene, tissue)
  if (!is.null(em_iteration)) cases$em_iteration <- as.integer(em_iteration)
  dir.create(plot_dir, recursive = TRUE, showWarnings = FALSE)
  plot_summary_file <- file.path(plot_dir, summary_filename)
  fwrite(cases, file.path(plot_dir, "selected_cases.csv"))
  empty_summary <- data.table(gene = character(), tissue = character(),
                              status = character(), message = character(),
                              mixed_fit_name = character())
  if (!is.null(em_iteration)) empty_summary$em_iteration <- integer()
  # Reset the audit before any expensive input loading. No cases need no GTEx.
  fwrite(empty_summary, plot_summary_file)
  if (!nrow(cases)) {
    message("No additive-1-CS / ", mixed_label, "-2-CS cases met the filters.")
    return(invisible(empty_summary))
  }
  message("Selected ", nrow(cases), " gene-tissue pairs (", uniqueN(cases$gene), " genes).")
  ...
  add_result <- function(result) {
    result$mixed_fit_name <- mixed_fit_name
    if (!is.null(em_iteration)) result$em_iteration <- as.integer(em_iteration)
      ...
      case_result <- tryCatch(
        {
          ...
          # Reconstruction checks dimensions against this field in a local copy.
  ...
  if (any(plot_summary$status == "error")) {
    warning(sum(plot_summary$status == "error"), " selected cases could not be plotted; see ", plot_summary_file)
  }
