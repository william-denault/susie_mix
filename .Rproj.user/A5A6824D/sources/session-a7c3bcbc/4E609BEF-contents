# Fit only weighted mixed-coding SuSiE, using one prior row per tissue.
# Called by run_em_chunk.R with the iteration's immutable priors.csv.
run_susie_gene <- function(
  ...
  analyze_tissue <- function(target_tissue) {
    ...
    n_add_rm <- sum(
      !keep_mix & geno_mix_coding_all == "additive"
    )
    ...
    if (ncol(geno_mix) == 0L) {
      cat("Skipping", target_tissue, "- no predictors remained after filtering.\n")
      return(NULL)
    }

    mix_coding_prior <- em_prior_for_tissue(tissue_priors, target_tissue)
    weighted_mix_prior_weights <- em_predictor_weights(
      mix_coding, mix_coding_prior, mix_predictor_names
    )
    ...
    stopifnot(identical(as.character(pheno$SUBJID), rownames(geno_mix)))
    ...
    list(
      ...
      # Lead biological SNP and signed distance to the TSS.
      weighted_fit_mix_lead_snp_tss_distance =
        weighted_fit_mix_lead_snp_tss_distance,
