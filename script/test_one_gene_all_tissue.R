...
#
# Modified: instead of fine-mapping in a single target_tissue, this
# version loops over ALL tissues available for the target gene and
# fine-maps in each one.
#
# sinteractive ...
# module load R/4.2.0
# R
...
target_gene   <- "CBX8"
# target_tissue removed -- we now loop over all tissues available for the gene
min_maf       <- 0.05
...
pheno_all <- fread(file.path(datadir,
                             "GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_reads.gct.gz"),
...
# All tissues available for this gene (instead of a single target_tissue).
...
geno_all <- fread("plink2.raw",sep = "\t",header = TRUE)
...
# Store per-tissue results and plots.
fits  <- list()
plots <- list()
...
# 'fits' and 'plots' are now lists keyed by tissue name, e.g.:
#   fits[["Lung"]]$sets
#   plots[["Lung"]]
