...
# n_rec == 1 means the sole mixed-model CS has a recessive lead predictor.
recessive_cases <- res[
  is.finite(min_pv) &
...
setorder(recessive_cases, min_pv, gene, tissue)

cat(
  "One-CS recessive fit_mix candidates:",
  nrow(recessive_cases),
  "\n"
)
