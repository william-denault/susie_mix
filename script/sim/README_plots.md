...
| `roc_pure_all_L`, `roc_mixed_all_L` | The same respective scenario rows | All causal counts overlaid: colour denotes method and line style denotes L; generated by default |
...
The true number of causal SNPs is `L_add + L_rec + L_dom`. It is different from the fitted upper bound `L = 10` in your jobs. Each per-L ROC title specifies the true count; its panels contain only the blue SuSiE curve and pink SuSiE-mix curve, with identical axis ranges across L. The additional all-L figures overlay the separate curves for each causal count, rather than pooling counts across L into one curve. Set `write_roc_all_L <- FALSE` to skip these overviews.
